...
## Purpose
Define portable builds, package contents, CI checks and release evidence for the supported platforms.
...
The first release SHALL produce packages for macOS, Linux and iOS, each containing the C header, the library, the licence and the attribution file, and each SHALL be smoke-tested by a program that links it. Windows and Android are tracked by the `windows-android-release` follow-up change.
