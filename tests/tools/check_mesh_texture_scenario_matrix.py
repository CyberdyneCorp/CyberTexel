...
MATRIX = ROOT / "docs" / "mesh-and-texture-sets-scenarios.md"
SPEC = (
    ROOT
    / "openspec"
    / "specs"
...
LABEL = "mesh-and-texture-sets-scenario"
...
def main() -> int:
    ...
    if "mesh-and-texture-sets-scenario-matrix" not in labeled:
        failures.append("scenario matrix CTest is missing its capability label")
    if failures:
        for failure in failures:
            print(failure)
        return 1
    print(f"ok: all {len(expected)} mesh-and-texture-sets scenarios have mapped evidence")
    return 0
