...
| Embedded profile | `png-io`, `layered-image-io`, `c-abi-image-io` | Embedded PNG/BMP sRGB metadata, bounded PNG, multipart-JPEG, TIFF, flat/layered PSD and BMP RGB ICC profiles, OpenEXR chromaticities and Radiance primaries are interpreted when they describe supported Rec. 709 spaces; an explicit caller declaration overrides them. |
| Uninterpretable profile | `png-io`, `c-abi-image-io` | Unsupported or malformed embedded profiles, primaries, chromaticities and colour identifiers are reported before applying the automatic semantic rule. |
