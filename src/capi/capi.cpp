...
struct ctex_layer_snapshot {
    ctex_layer_snapshot(ctex_allocator_state allocator_value,
                        ctex::doc::LayerCompositeRequest value_value)
        : allocator(allocator_value), value(std::move(value_value)) {}
...
struct ctex_material_graph_workspace {
    explicit ctex_material_graph_workspace(ctex_allocator_state allocator_value)
        : allocator(allocator_value) {}

    ctex_allocator_state allocator;
    ctex::graph::GraphWorkspace value;
};

struct ctex_material_graph_node_registry {
    explicit ctex_material_graph_node_registry(ctex_allocator_state allocator_value)
        : allocator(allocator_value) {}

    ctex_allocator_state allocator;
    ctex::graph::NodeTypeRegistry value;
};

struct ctex_shader_emission_cache {
    explicit ctex_shader_emission_cache(ctex_allocator_state allocator_value)
        : allocator(allocator_value) {}
...
ctex_material_graph_workspace* create_material_graph_workspace(
    const ctex_allocator_state& allocator) {
    void* storage = allocate_storage(allocator, sizeof(ctex_material_graph_workspace),
                                     alignof(ctex_material_graph_workspace));
    try {
        return ::new (storage) ctex_material_graph_workspace(allocator);
    } catch (...) {
        deallocate_storage(allocator, storage, sizeof(ctex_material_graph_workspace),
                           alignof(ctex_material_graph_workspace));
        throw;
    }
}

ctex_material_graph_node_registry* create_material_graph_node_registry(
    const ctex_allocator_state& allocator) {
    void* storage = allocate_storage(allocator, sizeof(ctex_material_graph_node_registry),
                                     alignof(ctex_material_graph_node_registry));
    try {
        return ::new (storage) ctex_material_graph_node_registry(allocator);
    } catch (...) {
        deallocate_storage(allocator, storage, sizeof(ctex_material_graph_node_registry),
                           alignof(ctex_material_graph_node_registry));
        throw;
    }
}

ctex_shader_emission_cache* create_shader_emission_cache(const ctex_allocator_state& allocator) {
    void* storage = allocate_storage(allocator, sizeof(ctex_shader_emission_cache),
                                     alignof(ctex_shader_emission_cache));
    try {
        return ::new (storage) ctex_shader_emission_cache(allocator);
    } catch (...) {
        deallocate_storage(allocator, storage, sizeof(ctex_shader_emission_cache),
                           alignof(ctex_shader_emission_cache));
        throw;
    }
}
...
ctex_layer_snapshot* create_layer_snapshot(const ctex_allocator_state& allocator,
                                           ctex::doc::LayerCompositeRequest value) {
    void* storage =
        allocate_storage(allocator, sizeof(ctex_layer_snapshot), alignof(ctex_layer_snapshot));
    try {
        return ::new (storage) ctex_layer_snapshot(allocator, std::move(value));
    } catch (...) {
...
ctex::graph::SocketType material_graph_socket_type(std::uint32_t type) {
    if (type > CTEX_SMART_MATERIAL_VALUE_BOOLEAN) {
        throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_INVALID_ENUM_VALUE,
                       "material graph socket type=" + std::to_string(type));
    }
    return static_cast<ctex::graph::SocketType>(type);
}

std::vector<ctex::graph::NodeSocket> material_graph_group_sockets(
    const ctex_material_graph_socket_descriptor* descriptors, std::size_t count,
    bool retain_defaults) {
    if (descriptors == nullptr && count != 0) {
        throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                       "material graph group sockets=null");
    }
    std::vector<ctex::graph::NodeSocket> sockets;
    sockets.reserve(count);
    for (std::size_t index = 0; index < count; ++index) {
        const ctex_material_graph_socket_descriptor& descriptor = descriptors[index];
        validate_structure_size(descriptor.size, CTEX_MATERIAL_GRAPH_SOCKET_DESCRIPTOR_V1_SIZE,
                                CTEX_MATERIAL_GRAPH_SOCKET_DESCRIPTOR_CURRENT_SIZE,
                                "material graph socket descriptor size");
        if (descriptor.identifier == nullptr || descriptor.display_name == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "material graph socket identifier and display_name are required");
        }
        ctex::graph::SocketValue value;
        if (retain_defaults && descriptor.default_value != nullptr) {
            value = material_graph_value(descriptor.default_value);
        }
        sockets.push_back({.identifier = descriptor.identifier,
                           .display_name = descriptor.display_name,
                           .type = material_graph_socket_type(descriptor.type),
                           .value = std::move(value)});
    }
    return sockets;
}

ctex::graph::GraphDocument& material_graph_workspace_graph(ctex::graph::GraphWorkspace& workspace,
                                                           std::uint32_t owner_kind,
                                                           std::string_view owner_identifier) {
    switch (owner_kind) {
        case CTEX_MATERIAL_GRAPH_OWNER_MATERIAL:
            return workspace.material(owner_identifier);
        case CTEX_MATERIAL_GRAPH_OWNER_GROUP:
            return workspace.group_graph(owner_identifier);
        default:
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_INVALID_ENUM_VALUE,
                           "material graph owner kind=" + std::to_string(owner_kind));
    }
}

const ctex::graph::GraphDocument& material_graph_workspace_graph(
    const ctex::graph::GraphWorkspace& workspace, std::uint32_t owner_kind,
    std::string_view owner_identifier) {
    switch (owner_kind) {
        case CTEX_MATERIAL_GRAPH_OWNER_MATERIAL:
            return workspace.material(owner_identifier);
...
ctex::graph::EmissionTarget material_graph_emission_target(std::uint32_t target) {
    switch (target) {
        case CTEX_MATERIAL_GRAPH_TARGET_WGSL:
            return ctex::graph::EmissionTarget::wgsl;
        case CTEX_MATERIAL_GRAPH_TARGET_MSL:
            return ctex::graph::EmissionTarget::msl;
        case CTEX_MATERIAL_GRAPH_TARGET_SPIRV:
            return ctex::graph::EmissionTarget::spirv;
        case CTEX_MATERIAL_GRAPH_TARGET_HLSL:
...
ctex_smart_material_value_descriptor material_graph_callback_value(
    const ctex::graph::SocketValue& value) {
    ctex_smart_material_value_descriptor result{};
    result.size = CTEX_SMART_MATERIAL_VALUE_DESCRIPTOR_CURRENT_SIZE;
    if (const auto* scalar = std::get_if<double>(&value)) {
        result.type = CTEX_SMART_MATERIAL_VALUE_SCALAR;
        result.scalar = *scalar;
    } else if (const auto* vector = std::get_if<ctex::graph::VectorValue>(&value)) {
        result.type = CTEX_SMART_MATERIAL_VALUE_VECTOR;
        result.vector = {vector->x, vector->y, vector->z};
    } else if (const auto* colour = std::get_if<ctex::graph::ColourValue>(&value)) {
        result.type = CTEX_SMART_MATERIAL_VALUE_COLOUR;
        result.colour = {colour->r, colour->g, colour->b, colour->a};
    } else if (const auto* text = std::get_if<std::string>(&value)) {
        result.type = CTEX_SMART_MATERIAL_VALUE_STRING;
        result.text = text->c_str();
    } else if (const auto* image = std::get_if<ctex::graph::ImageValue>(&value)) {
        result.type = CTEX_SMART_MATERIAL_VALUE_IMAGE;
        result.text = image->resource_id.c_str();
    } else if (const auto* boolean = std::get_if<bool>(&value)) {
        result.type = CTEX_SMART_MATERIAL_VALUE_BOOLEAN;
        result.boolean = *boolean ? 1U : 0U;
    } else {
        throw ctex::graph::NodeInvocationError("host node callback received an empty value");
    }
    return result;
}

std::vector<ctex_material_graph_host_property_value> material_graph_callback_properties(
    const ctex::graph::GraphNode& node) {
    std::vector<ctex_material_graph_host_property_value> result;
    result.reserve(node.properties.size());
    for (const ctex::graph::NodeProperty& property : node.properties) {
        result.push_back({.identifier = property.key.c_str(),
                          .value = material_graph_callback_value(property.value)});
    }
    return result;
}

template <typename Callback>
ctex_result invoke_material_graph_host_callback(Callback&& callback) {
    const DiagnosticState saved = last_diagnostic;
    try {
        const ctex_result result = callback();
        last_diagnostic = saved;
        return result;
    } catch (...) {
        last_diagnostic = saved;
        throw;
...
std::vector<ctex::graph::SocketValue> material_graph_fixture_values(
    const ctex_smart_material_value_descriptor* values, std::size_t count, std::string_view name) {
    if (values == nullptr && count != 0) {
        throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                       std::string(name) + "=null");
    }
    std::vector<ctex::graph::SocketValue> result;
    result.reserve(count);
    for (std::size_t index = 0; index < count; ++index) {
        result.push_back(material_graph_value(&values[index]));
    }
    return result;
}

std::vector<ctex::graph::NodePropertyDeclaration> material_graph_host_properties(
    const ctex_material_graph_property_descriptor* properties, std::size_t count) {
    if (properties == nullptr && count != 0) {
        throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                       "material graph host properties=null");
    }
    std::vector<ctex::graph::NodePropertyDeclaration> result;
    result.reserve(count);
    for (std::size_t index = 0; index < count; ++index) {
        const ctex_material_graph_property_descriptor& property = properties[index];
        validate_structure_size(property.size, CTEX_MATERIAL_GRAPH_PROPERTY_DESCRIPTOR_V1_SIZE,
                                CTEX_MATERIAL_GRAPH_PROPERTY_DESCRIPTOR_CURRENT_SIZE,
                                "material graph property descriptor size");
        if (property.identifier == nullptr || property.display_name == nullptr ||
            property.default_value == nullptr ||
            (property.allowed_values == nullptr && property.allowed_value_count != 0)) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "host property declaration contains a null field");
        }
        std::vector<std::string> allowed;
        allowed.reserve(property.allowed_value_count);
        for (std::size_t choice = 0; choice < property.allowed_value_count; ++choice) {
            if (property.allowed_values[choice] == nullptr) {
                throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                               "host property allowed value is null");
            }
            allowed.emplace_back(property.allowed_values[choice]);
        }
        result.push_back({.identifier = property.identifier,
                          .display_name = property.display_name,
                          .default_value = material_graph_value(property.default_value),
                          .allowed_values = std::move(allowed)});
    }
    return result;
}

std::vector<ctex::graph::NodeResourceDependency> material_graph_host_dependencies(
    const char* const* dependencies, std::size_t count) {
    if (dependencies == nullptr && count != 0) {
        throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                       "resource_dependencies=null");
    }
    std::vector<ctex::graph::NodeResourceDependency> result;
    result.reserve(count);
    for (std::size_t index = 0; index < count; ++index) {
        if (dependencies[index] == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "resource dependency is null");
        }
        result.push_back({dependencies[index]});
    }
    return result;
}

std::vector<ctex::graph::EmissionTarget> material_graph_host_targets(const std::uint32_t* targets,
                                                                     std::size_t count) {
    if (targets == nullptr && count != 0) {
        throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                       "supported_targets=null");
    }
    std::vector<ctex::graph::EmissionTarget> result;
    result.reserve(count);
    for (std::size_t index = 0; index < count; ++index) {
        result.push_back(material_graph_emission_target(targets[index]));
    }
    return result;
}

std::vector<ctex::graph::NodeParityFixture> material_graph_host_fixtures(
    const ctex_material_graph_parity_fixture_descriptor* fixtures, std::size_t count) {
    if (fixtures == nullptr && count != 0) {
        throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                       "parity_fixtures=null");
    }
    std::vector<ctex::graph::NodeParityFixture> result;
    result.reserve(count);
    for (std::size_t index = 0; index < count; ++index) {
        const ctex_material_graph_parity_fixture_descriptor& fixture = fixtures[index];
        validate_structure_size(fixture.size, CTEX_MATERIAL_GRAPH_PARITY_FIXTURE_DESCRIPTOR_V1_SIZE,
                                CTEX_MATERIAL_GRAPH_PARITY_FIXTURE_DESCRIPTOR_CURRENT_SIZE,
                                "material graph parity fixture size");
        if (fixture.identifier == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "parity fixture identifier=null");
        }
        result.push_back({.identifier = fixture.identifier,
                          .inputs = material_graph_fixture_values(
                              fixture.inputs, fixture.input_count, "parity fixture inputs"),
                          .expected_outputs = material_graph_fixture_values(
                              fixture.expected_outputs, fixture.expected_output_count,
                              "parity fixture expected_outputs"),
                          .tolerance = fixture.tolerance});
    }
    return result;
}

std::vector<ctex::graph::SocketValue> invoke_material_graph_cpu_callback(
    ctex_material_graph_cpu_evaluate_callback callback, void* user_data, std::size_t output_count,
    const ctex::graph::NodeEvaluationRequest& request) {
    const auto properties = material_graph_callback_properties(request.node);
    std::vector<ctex_smart_material_value_descriptor> inputs;
    inputs.reserve(request.inputs.size());
    for (const ctex::graph::SocketValue& value : request.inputs) {
        inputs.push_back(material_graph_callback_value(value));
    }
    std::vector<ctex_smart_material_value_descriptor> outputs(output_count);
    for (ctex_smart_material_value_descriptor& output : outputs) {
        output.size = CTEX_SMART_MATERIAL_VALUE_DESCRIPTOR_CURRENT_SIZE;
    }
    const ctex_material_graph_host_evaluation_request callback_request = {
        .size = CTEX_MATERIAL_GRAPH_HOST_EVALUATION_REQUEST_CURRENT_SIZE,
        .node_id = request.node.id,
        .type_id = request.node.type_id.c_str(),
        .type_version = request.node.type_version,
        .properties = properties.data(),
        .property_count = properties.size(),
        .inputs = inputs.data(),
        .input_count = inputs.size(),
    };
    const ctex_result callback_result = invoke_material_graph_host_callback(
        [&] { return callback(&callback_request, outputs.data(), outputs.size(), user_data); });
    if (callback_result != CTEX_RESULT_SUCCESS) {
        throw ctex::graph::NodeInvocationError("CPU callback returned result " +
                                               std::to_string(callback_result));
    }
    std::vector<ctex::graph::SocketValue> result;
    result.reserve(outputs.size());
    for (const ctex_smart_material_value_descriptor& output : outputs) {
        result.push_back(material_graph_value(&output));
    }
    return result;
}

ctex::graph::NodeEmissionResult invoke_material_graph_emit_callback(
    ctex_material_graph_emit_callback callback, void* user_data,
    const ctex::graph::NodeEmissionRequest& request) {
    const auto properties = material_graph_callback_properties(request.node);
    std::vector<const char*> input_expressions;
    input_expressions.reserve(request.input_expressions.size());
    for (const std::string& expression : request.input_expressions) {
        input_expressions.push_back(expression.c_str());
    }
    const ctex_material_graph_host_emission_request callback_request = {
        .size = CTEX_MATERIAL_GRAPH_HOST_EMISSION_REQUEST_CURRENT_SIZE,
        .node_id = request.node.id,
        .type_id = request.node.type_id.c_str(),
        .type_version = request.node.type_version,
        ...
        .properties = properties.data(),
        .property_count = properties.size(),
        .input_expressions = input_expressions.data(),
        .input_expression_count = input_expressions.size(),
    };
    ctex_material_graph_host_emission_result callback_output{};
    callback_output.size = CTEX_MATERIAL_GRAPH_HOST_EMISSION_RESULT_CURRENT_SIZE;
    const ctex_result callback_result = invoke_material_graph_host_callback(
        [&] { return callback(&callback_request, &callback_output, user_data); });
    if (callback_result != CTEX_RESULT_SUCCESS) {
        throw ctex::graph::NodeInvocationError("emission callback returned result " +
                                               std::to_string(callback_result));
    }
    validate_structure_size(callback_output.size, CTEX_MATERIAL_GRAPH_HOST_EMISSION_RESULT_V1_SIZE,
                            CTEX_MATERIAL_GRAPH_HOST_EMISSION_RESULT_CURRENT_SIZE,
                            "host emission result size");
    if ((callback_output.output_expressions == nullptr &&
         callback_output.output_expression_count != 0) ||
        (callback_output.resource_identifiers == nullptr &&
         callback_output.resource_identifier_count != 0)) {
        throw ctex::graph::NodeInvocationError("emission callback returned a null string array");
    }
    ctex::graph::NodeEmissionResult result;
    result.output_expressions.reserve(callback_output.output_expression_count);
    result.resource_identifiers.reserve(callback_output.resource_identifier_count);
    for (std::size_t index = 0; index < callback_output.output_expression_count; ++index) {
        if (callback_output.output_expressions[index] == nullptr) {
            throw ctex::graph::NodeInvocationError("emission callback returned a null expression");
        }
        result.output_expressions.emplace_back(callback_output.output_expressions[index]);
    }
    for (std::size_t index = 0; index < callback_output.resource_identifier_count; ++index) {
        if (callback_output.resource_identifiers[index] == nullptr) {
            throw ctex::graph::NodeInvocationError("emission callback returned a null resource");
        }
        result.resource_identifiers.emplace_back(callback_output.resource_identifiers[index]);
    }
    return result;
}

ctex::graph::HostNodeTypeRegistration material_graph_host_registration(
    const ctex_material_graph_host_node_registration_descriptor& descriptor) {
    if (descriptor.type_id == nullptr || descriptor.display_name == nullptr) {
        throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                       "host node type_id and display_name are required");
    }
    if (descriptor.deterministic > 1U) {
        throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_INVALID_DESCRIPTOR_VALUE,
                       "host node deterministic must be zero or one");
    }
    ctex::graph::HostNodeTypeRegistration result{
        .declaration = {.type_id = descriptor.type_id,
                        .version = descriptor.type_version,
                        .display_name = descriptor.display_name,
                        .category = ctex::graph::NodeCategory::host,
                        .inputs = material_graph_group_sockets(descriptor.inputs,
                                                               descriptor.input_count, true),
                        .outputs = material_graph_group_sockets(descriptor.outputs,
                                                                descriptor.output_count, false),
                        .properties = material_graph_host_properties(descriptor.properties,
                                                                     descriptor.property_count)},
        .cpu_evaluate = {},
        .emit = {},
        .deterministic = descriptor.deterministic != 0,
        .resource_dependencies = material_graph_host_dependencies(
            descriptor.resource_dependencies, descriptor.resource_dependency_count),
        .supported_targets = material_graph_host_targets(descriptor.supported_targets,
                                                         descriptor.supported_target_count),
        .parity_fixtures = material_graph_host_fixtures(descriptor.parity_fixtures,
                                                        descriptor.parity_fixture_count),
    };
    if (descriptor.cpu_evaluate != nullptr) {
        result.cpu_evaluate = [callback = descriptor.cpu_evaluate, user_data = descriptor.user_data,
                               output_count = descriptor.output_count](
                                  const ctex::graph::NodeEvaluationRequest& request) {
            return invoke_material_graph_cpu_callback(callback, user_data, output_count, request);
        };
    }
    if (descriptor.emit != nullptr) {
        result.emit = [callback = descriptor.emit, user_data = descriptor.user_data](
                          const ctex::graph::NodeEmissionRequest& request) {
            return invoke_material_graph_emit_callback(callback, user_data, request);
        };
    }
    return result;
}

std::string material_graph_host_contract_json(const ctex::graph::NodeParityReport& parity,
                                              const ctex::graph::NodeReplayEligibility& replay) {
    std::string output = "{\"parity_passed\":";
    output += parity.passed() ? "true" : "false";
    output += ",\"parity_failures\":[";
    for (std::size_t index = 0; index < parity.failures.size(); ++index) {
        if (index != 0) {
            output.push_back(',');
        }
        output += "{\"fixture\":";
        append_json_text(output, parity.failures[index].fixture_identifier);
        output += ",\"detail\":";
        append_json_text(output, parity.failures[index].detail);
        output.push_back('}');
    }
    output += "],\"replay_eligible\":";
    output += replay.eligible ? "true" : "false";
    output += ",\"unpinned_dependencies\":[";
    for (std::size_t index = 0; index < replay.unpinned_dependencies.size(); ++index) {
        if (index != 0) {
            output.push_back(',');
        }
        append_json_text(output, replay.unpinned_dependencies[index]);
    }
    output += "],\"reason\":";
    append_json_text(output, replay.reason);
    output += "}";
    return output;
}
...
ctex::emit::LayerStackEmissionRequest shader_layer_stack_request(
    const ctex_shader_layer_stack_request& descriptor) {
    validate_structure_size(descriptor.size, CTEX_SHADER_LAYER_STACK_REQUEST_V1_SIZE,
                            CTEX_SHADER_LAYER_STACK_REQUEST_CURRENT_SIZE,
                            "shader layer-stack request size");
    if (descriptor.stable_identity == nullptr ||
        (descriptor.layers == nullptr && descriptor.layer_count != 0)) {
        throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                       "stable_identity and layer storage are required");
    }
    if (descriptor.requested_filter > CTEX_SHADER_FILTER_LINEAR) {
        throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_INVALID_ENUM_VALUE,
                       "requested_filter=" + std::to_string(descriptor.requested_filter));
    }
    ctex::emit::LayerStackEmissionRequest result{
        .stable_identity = descriptor.stable_identity,
        .target = material_graph_emission_target(descriptor.target),
        .features = shader_features(descriptor.features),
        .layers = {},
        .output = shader_texture(descriptor.output, "shader layer-stack output"),
        .requested_filter = static_cast<ctex::emit::FilterMode>(descriptor.requested_filter),
    };
    result.layers.reserve(descriptor.layer_count);
    for (std::size_t index = 0; index < descriptor.layer_count; ++index) {
        const auto& layer = descriptor.layers[index];
        validate_structure_size(layer.size, CTEX_SHADER_LAYER_DESCRIPTOR_V1_SIZE,
                                CTEX_SHADER_LAYER_DESCRIPTOR_CURRENT_SIZE,
                                "shader layer descriptor size");
        if (layer.identifier == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "shader layer identifier=null");
        }
        result.layers.push_back(
            {layer.identifier, shader_texture(layer.texture, "shader layer texture")});
    }
    return result;
}
...
void return_shader_material_emission(const PreparedShaderEmission& prepared, std::uint32_t target,
                                     ctex_shader_material_info* out_info, void* vertex_artifact,
...
struct PreparedLayerStackEmission {
    std::vector<std::byte> artifacts;
    std::string artifact_report;
    std::string pass_plan;
    std::string workarounds;
};

struct ShaderArtifactRange {
...
void append_shader_artifact(std::vector<std::byte>& destination,
                            std::span<const std::byte> artifact) {
    destination.insert(destination.end(), artifact.begin(), artifact.end());
}

ShaderArtifactRange append_shader_program(std::vector<std::byte>& destination,
                                          const ctex::emit::KongShaderProgram& program) {
    ShaderArtifactRange range{.vertex_offset = destination.size()};
    std::visit(
        [&](const auto& payload) {
            using Payload = std::decay_t<decltype(payload)>;
            if constexpr (std::is_same_v<Payload, ctex::emit::SplitTextShaderProgram>) {
                const auto vertex = shader_text_artifact(payload.vertex_source);
                const auto fragment = shader_text_artifact(payload.fragment_source);
                range.vertex_size = vertex.size();
                append_shader_artifact(destination, vertex);
                range.fragment_offset = destination.size();
                range.fragment_size = fragment.size();
                append_shader_artifact(destination, fragment);
                range.encoding = "text";
            } else if constexpr (std::is_same_v<Payload, ctex::emit::UnifiedTextShaderProgram>) {
                const auto source = shader_text_artifact(payload.source);
                range.vertex_size = source.size();
                append_shader_artifact(destination, source);
                range.fragment_offset = destination.size();
                range.encoding = "text";
            } else {
                range.vertex_size = payload.vertex_module.size() * sizeof(std::uint32_t);
                append_shader_artifact(
                    destination, {reinterpret_cast<const std::byte*>(payload.vertex_module.data()),
                                  range.vertex_size});
                range.fragment_offset = destination.size();
                range.fragment_size = payload.fragment_module.size() * sizeof(std::uint32_t);
                append_shader_artifact(
                    destination,
                    {reinterpret_cast<const std::byte*>(payload.fragment_module.data()),
                     range.fragment_size});
                range.encoding = "spirv";
            }
        },
        program.payload);
    return range;
...
PreparedLayerStackEmission prepare_layer_stack_emission(
    const ctex::emit::LayerStackEmission& emission) {
    PreparedLayerStackEmission result;
    result.artifact_report = "[";
    for (std::size_t index = 0; index < emission.shaders.size(); ++index) {
        const ctex::emit::EmittedLayerStackPass& shader = emission.shaders[index];
        const ShaderArtifactRange range = append_shader_program(result.artifacts, shader.shader);
        if (index != 0) result.artifact_report.push_back(',');
        result.artifact_report += "{\"pass_identifier\":";
        append_json_text(result.artifact_report, shader.pass_identifier);
        result.artifact_report += ",\"target\":";
        append_json_text(result.artifact_report,
                         ctex::emit::shader_target_name(shader.shader.target));
        result.artifact_report += ",\"encoding\":";
        append_json_text(result.artifact_report, range.encoding);
        result.artifact_report += ",\"vertex_offset\":" + std::to_string(range.vertex_offset) +
                                  ",\"vertex_size\":" + std::to_string(range.vertex_size) +
                                  ",\"fragment_offset\":" + std::to_string(range.fragment_offset) +
                                  ",\"fragment_size\":" + std::to_string(range.fragment_size) + "}";
    }
    result.artifact_report.push_back(']');
    result.pass_plan = shader_pass_plan_json(emission.pass_plan);
    result.workarounds = shader_workaround_json(emission.workarounds);
    return result;
}

void return_layer_stack_emission(const ctex::emit::LayerStackEmission& emission,
                                 const PreparedLayerStackEmission& prepared,
                                 const ctex_shader_layer_stack_request& request, bool cache_hit,
                                 ctex_shader_layer_stack_info* out_info, void* artifact_blob,
                                 std::size_t artifact_blob_size, char* artifact_report_output,
                                 std::size_t artifact_report_output_size, char* pass_plan_output,
                                 std::size_t pass_plan_output_size, char* workaround_report_output,
                                 std::size_t workaround_report_output_size) {
    *out_info = {
        .size = CTEX_SHADER_LAYER_STACK_INFO_CURRENT_SIZE,
        .target = request.target,
        .layer_count = request.layer_count,
        .pass_count = emission.pass_plan.passes().size(),
        .artifact_blob_size = prepared.artifacts.size(),
        .artifact_report_size = prepared.artifact_report.size() + 1,
        .pass_plan_size = prepared.pass_plan.size() + 1,
        .workaround_report_size = prepared.workarounds.size() + 1,
        .workaround_count = emission.workarounds.size(),
        .compute_used = emission.compute_used ? 1U : 0U,
        .cache_hit = cache_hit ? 1U : 0U,
    };
    validate_output_array(artifact_blob, artifact_blob_size, prepared.artifacts.size(),
                          "artifact_blob");
    validate_string_buffer(artifact_report_output, artifact_report_output_size,
                           out_info->artifact_report_size);
    validate_string_buffer(pass_plan_output, pass_plan_output_size, out_info->pass_plan_size);
    validate_string_buffer(workaround_report_output, workaround_report_output_size,
                           out_info->workaround_report_size);
    if (artifact_blob != nullptr && !prepared.artifacts.empty()) {
        std::memcpy(artifact_blob, prepared.artifacts.data(), prepared.artifacts.size());
    }
    if (artifact_report_output != nullptr) {
        std::memcpy(artifact_report_output, prepared.artifact_report.c_str(),
                    out_info->artifact_report_size);
    }
    if (pass_plan_output != nullptr) {
        std::memcpy(pass_plan_output, prepared.pass_plan.c_str(), out_info->pass_plan_size);
...
void publish_layer_composite(const ctex::doc::LayerCompositeResult& result,
                             ctex_layer_composite_info& info,
                             const LayerCompositeOutputBuffers& output,
                             std::size_t channel_capacity, std::size_t semantic_id_size,
    ...
    validate_output_array(output.channels, channel_capacity, result.channels.size(), "channels");
    validate_string_buffer(output.semantic_ids, semantic_id_size, required_semantic_size);
    validate_output_array(output.pixels, pixel_capacity, shape.pixel_count, "pixels");
    info = {
    ...
    write_layer_composite_output(result, output);
}

struct LayerSnapshotShape {
    std::size_t string_size{};
    std::size_t pixel_count{};
    std::size_t coverage_count{};
    std::size_t mask_value_count{};
};

void add_layer_snapshot_size(std::size_t value, std::size_t& total, std::string_view name) {
    if (value > std::numeric_limits<std::size_t>::max() - total) {
        throw std::overflow_error(std::string("layer snapshot ") + std::string(name) +
                                  " count overflow");
    }
    total += value;
}

LayerSnapshotShape layer_snapshot_shape(const ctex::doc::LayerCompositeRequest& snapshot) {
    LayerSnapshotShape shape;
    for (const ctex::doc::LayerCompositeRaster& raster : snapshot.content) {
        add_layer_snapshot_size(raster.entry_identifier.size() + 1, shape.string_size, "string");
        add_layer_snapshot_size(raster.semantic_id.size() + 1, shape.string_size, "string");
        add_layer_snapshot_size(raster.pixels.size(), shape.pixel_count, "pixel");
        add_layer_snapshot_size(raster.coverage.size(), shape.coverage_count, "coverage");
    }
    for (const ctex::doc::LayerCompositeMaskRaster& mask : snapshot.masks) {
        add_layer_snapshot_size(mask.mask_identifier.size() + 1, shape.string_size, "string");
        add_layer_snapshot_size(mask.values.size(), shape.mask_value_count, "mask value");
    }
    return shape;
}

struct LayerSnapshotOutputBuffers {
    ctex_layer_snapshot_content_info* content{};
    ctex_layer_snapshot_mask_info* masks{};
    char* strings{};
    ctex_vec4f* pixels{};
    float* coverage{};
    double* mask_values{};
...
void write_layer_snapshot_content(const ctex::doc::LayerCompositeRaster& raster, std::size_t index,
                                  std::size_t& string_offset, std::size_t& pixel_offset,
                                  std::size_t& coverage_offset,
                                  const LayerSnapshotOutputBuffers& output) {
    const std::size_t entry_size = raster.entry_identifier.size() + 1;
    const std::size_t semantic_offset = string_offset + entry_size;
    const std::size_t semantic_size = raster.semantic_id.size() + 1;
    if (output.content != nullptr) {
        output.content[index] = {
            .width = raster.width,
            .height = raster.height,
            .entry_identifier_offset = string_offset,
            .entry_identifier_size = entry_size,
            .semantic_id_offset = semantic_offset,
            .semantic_id_size = semantic_size,
            .pixel_offset = pixel_offset,
            .pixel_count = raster.pixels.size(),
            .coverage_offset = coverage_offset,
            .coverage_count = raster.coverage.size(),
        };
    }
    if (output.strings != nullptr) {
        std::memcpy(output.strings + string_offset, raster.entry_identifier.c_str(), entry_size);
        std::memcpy(output.strings + semantic_offset, raster.semantic_id.c_str(), semantic_size);
    }
    if (output.pixels != nullptr) {
        for (std::size_t pixel = 0; pixel < raster.pixels.size(); ++pixel) {
            const ctex::graph::ColourValue value = raster.pixels[pixel];
            output.pixels[pixel_offset + pixel] = {value.r, value.g, value.b, value.a};
        }
    }
    if (output.coverage != nullptr) {
        std::copy(raster.coverage.begin(), raster.coverage.end(),
                  output.coverage + coverage_offset);
    }
    string_offset = semantic_offset + semantic_size;
    pixel_offset += raster.pixels.size();
    coverage_offset += raster.coverage.size();
}

void write_layer_snapshot_mask(const ctex::doc::LayerCompositeMaskRaster& mask, std::size_t index,
                               std::size_t& string_offset, std::size_t& value_offset,
                               const LayerSnapshotOutputBuffers& output) {
    const std::size_t identifier_size = mask.mask_identifier.size() + 1;
    if (output.masks != nullptr) {
        output.masks[index] = {
            .width = mask.width,
            .height = mask.height,
            .mask_identifier_offset = string_offset,
            .mask_identifier_size = identifier_size,
            .value_offset = value_offset,
            .value_count = mask.values.size(),
        };
    }
    if (output.strings != nullptr) {
        std::memcpy(output.strings + string_offset, mask.mask_identifier.c_str(), identifier_size);
    }
    if (output.mask_values != nullptr) {
        std::copy(mask.values.begin(), mask.values.end(), output.mask_values + value_offset);
    }
    string_offset += identifier_size;
    value_offset += mask.values.size();
}

void write_layer_snapshot(const ctex::doc::LayerCompositeRequest& snapshot,
                          const LayerSnapshotOutputBuffers& output) {
    std::size_t string_offset = 0;
    std::size_t pixel_offset = 0;
    std::size_t coverage_offset = 0;
    for (std::size_t index = 0; index < snapshot.content.size(); ++index) {
        write_layer_snapshot_content(snapshot.content[index], index, string_offset, pixel_offset,
                                     coverage_offset, output);
    }
    std::size_t mask_value_offset = 0;
    for (std::size_t index = 0; index < snapshot.masks.size(); ++index) {
        write_layer_snapshot_mask(snapshot.masks[index], index, string_offset, mask_value_offset,
                                  output);
    }
}

void publish_layer_snapshot(const ctex::doc::LayerCompositeRequest& snapshot,
                            ctex_layer_snapshot_info& info,
                            const LayerSnapshotOutputBuffers& output, std::size_t content_capacity,
                            std::size_t mask_capacity, std::size_t string_size,
                            std::size_t pixel_capacity, std::size_t coverage_capacity,
                            std::size_t mask_value_capacity) {
    const LayerSnapshotShape shape = layer_snapshot_shape(snapshot);
    validate_output_array(output.content, content_capacity, snapshot.content.size(), "content");
    validate_output_array(output.masks, mask_capacity, snapshot.masks.size(), "masks");
    validate_string_buffer(output.strings, string_size, shape.string_size);
    validate_output_array(output.pixels, pixel_capacity, shape.pixel_count, "pixels");
    validate_output_array(output.coverage, coverage_capacity, shape.coverage_count, "coverage");
    validate_output_array(output.mask_values, mask_value_capacity, shape.mask_value_count,
                          "mask_values");
    info = {
        .size = CTEX_LAYER_SNAPSHOT_INFO_CURRENT_SIZE,
        .width = snapshot.width,
        .height = snapshot.height,
        .content_count = snapshot.content.size(),
        .mask_count = snapshot.masks.size(),
        .required_string_size = shape.string_size,
        .required_pixel_count = shape.pixel_count,
        .required_coverage_count = shape.coverage_count,
        .required_mask_value_count = shape.mask_value_count,
    };
...
extern "C" ctex_result ctex_material_graph_workspace_create(
    ctex_material_graph_workspace** out_workspace) {
    return call_material_graph_boundary("ctex_material_graph_workspace_create", [&] {
        if (out_workspace == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "out_workspace=null");
        }
        *out_workspace = nullptr;
        *out_workspace = create_material_graph_workspace(current_allocator());
    });
}

extern "C" void ctex_material_graph_workspace_destroy(ctex_material_graph_workspace* workspace) {
    if (workspace == nullptr) {
        return;
    }
    const ctex_allocator_state allocator = workspace->allocator;
    workspace->~ctex_material_graph_workspace();
    deallocate_storage(allocator, workspace, sizeof(ctex_material_graph_workspace),
                       alignof(ctex_material_graph_workspace));
}

extern "C" ctex_result ctex_material_graph_workspace_get_info(
    const ctex_material_graph_workspace* workspace, ctex_material_graph_workspace_info* out_info) {
    return call_material_graph_boundary("ctex_material_graph_workspace_get_info", [&] {
        if (workspace == nullptr || out_info == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           workspace == nullptr ? "workspace=null" : "out_info=null");
        }
        validate_structure_size(out_info->size, CTEX_MATERIAL_GRAPH_WORKSPACE_INFO_V1_SIZE,
                                CTEX_MATERIAL_GRAPH_WORKSPACE_INFO_CURRENT_SIZE,
                                "material graph workspace info size");
        *out_info = {.size = CTEX_MATERIAL_GRAPH_WORKSPACE_INFO_CURRENT_SIZE,
                     .material_count = workspace->value.materials().size(),
                     .group_count = workspace->value.groups().size()};
    });
}

extern "C" ctex_result ctex_material_graph_workspace_add_material(
    ctex_material_graph_workspace* workspace, const char* identifier, const void* serialized,
    std::size_t serialized_size) {
    return call_material_graph_boundary("ctex_material_graph_workspace_add_material", [&] {
        if (workspace == nullptr || identifier == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           workspace == nullptr ? "workspace=null" : "identifier=null");
        }
        ctex::graph::GraphWorkspace staged = workspace->value;
        staged.add_material(identifier, ctex::graph::deserialize_graph(
                                            material_graph_bytes(serialized, serialized_size)));
        workspace->value = std::move(staged);
    });
}

extern "C" ctex_result ctex_material_graph_workspace_create_group(
    ctex_material_graph_workspace* workspace,
    const ctex_material_graph_group_descriptor* descriptor) {
    return call_material_graph_boundary("ctex_material_graph_workspace_create_group", [&] {
        if (workspace == nullptr || descriptor == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           workspace == nullptr ? "workspace=null" : "descriptor=null");
        }
        validate_structure_size(descriptor->size, CTEX_MATERIAL_GRAPH_GROUP_DESCRIPTOR_V1_SIZE,
                                CTEX_MATERIAL_GRAPH_GROUP_DESCRIPTOR_CURRENT_SIZE,
                                "material graph group descriptor size");
        if (descriptor->identifier == nullptr || descriptor->display_name == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "group identifier and display_name are required");
        }
        std::vector inputs =
            material_graph_group_sockets(descriptor->inputs, descriptor->input_count, true);
        std::vector outputs =
            material_graph_group_sockets(descriptor->outputs, descriptor->output_count, false);
        ctex::graph::GraphWorkspace staged = workspace->value;
        staged.create_group(descriptor->identifier, descriptor->display_name, std::move(inputs),
                            std::move(outputs));
        workspace->value = std::move(staged);
    });
}

extern "C" ctex_result ctex_material_graph_workspace_instantiate_group(
    ctex_material_graph_workspace* workspace, const char* group_identifier,
    std::uint32_t owner_kind, const char* owner_identifier, ctex_vec2f position,
    std::uint64_t* out_node_id) {
    return call_material_graph_boundary("ctex_material_graph_workspace_instantiate_group", [&] {
        if (workspace == nullptr || group_identifier == nullptr || owner_identifier == nullptr ||
            out_node_id == nullptr) {
            throw_boundary(
                CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                "workspace, group_identifier, owner_identifier and out_node_id are required");
        }
        ctex::graph::GraphWorkspace staged = workspace->value;
        ctex::graph::NodeId node_id{};
        if (owner_kind == CTEX_MATERIAL_GRAPH_OWNER_MATERIAL) {
            node_id = staged.instantiate_group_in_material(group_identifier, owner_identifier,
                                                           {position.x, position.y});
        } else if (owner_kind == CTEX_MATERIAL_GRAPH_OWNER_GROUP) {
            node_id = staged.instantiate_group_in_group(group_identifier, owner_identifier,
                                                        {position.x, position.y});
        } else {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_INVALID_ENUM_VALUE,
                           "material graph owner kind=" + std::to_string(owner_kind));
...
extern "C" ctex_result ctex_material_graph_workspace_update_group_interface(
    ctex_material_graph_workspace* workspace, const char* group_identifier,
    const ctex_material_graph_group_interface_descriptor* descriptor,
    ctex_material_graph_group_update_info* out_info) {
    return call_material_graph_boundary(
        "ctex_material_graph_workspace_update_group_interface", [&] {
            if (workspace == nullptr || group_identifier == nullptr || descriptor == nullptr ||
                out_info == nullptr) {
                throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                               "workspace, group_identifier, descriptor and out_info are required");
            }
            validate_structure_size(descriptor->size,
                                    CTEX_MATERIAL_GRAPH_GROUP_INTERFACE_DESCRIPTOR_V1_SIZE,
                                    CTEX_MATERIAL_GRAPH_GROUP_INTERFACE_DESCRIPTOR_CURRENT_SIZE,
                                    "material graph group interface descriptor size");
            validate_structure_size(out_info->size, CTEX_MATERIAL_GRAPH_GROUP_UPDATE_INFO_V1_SIZE,
                                    CTEX_MATERIAL_GRAPH_GROUP_UPDATE_INFO_CURRENT_SIZE,
                                    "material graph group update info size");
            std::vector inputs =
                material_graph_group_sockets(descriptor->inputs, descriptor->input_count, true);
            std::vector outputs =
                material_graph_group_sockets(descriptor->outputs, descriptor->output_count, false);
            ctex::graph::GraphWorkspace staged = workspace->value;
            const ctex::graph::GroupInterfaceUpdate update = staged.update_group_interface(
                group_identifier, std::move(inputs), std::move(outputs));
            const std::uint32_t version = staged.group(group_identifier).version;
            workspace->value = std::move(staged);
            *out_info = {.size = CTEX_MATERIAL_GRAPH_GROUP_UPDATE_INFO_CURRENT_SIZE,
                         .group_version = version,
                         .instances_updated = update.instances_updated,
                         .removed_link_count = update.removed_links.size()};
        });
}

extern "C" ctex_result ctex_material_graph_workspace_get_graph(
    const ctex_material_graph_workspace* workspace, std::uint32_t owner_kind,
    const char* owner_identifier, ctex_material_graph_info* out_info, void* canonical_output,
    std::size_t canonical_output_size, char* report_output, std::size_t report_output_size) {
    return call_material_graph_boundary("ctex_material_graph_workspace_get_graph", [&] {
        if (workspace == nullptr || owner_identifier == nullptr || out_info == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "workspace, owner_identifier and out_info are required");
        }
        validate_structure_size(out_info->size, CTEX_MATERIAL_GRAPH_INFO_V1_SIZE,
                                CTEX_MATERIAL_GRAPH_INFO_CURRENT_SIZE, "material graph info size");
        const PreparedMaterialGraph prepared = prepare_material_graph(
            material_graph_workspace_graph(workspace->value, owner_kind, owner_identifier));
        return_material_graph(prepared, *out_info, canonical_output, canonical_output_size,
                              report_output, report_output_size);
    });
}

extern "C" ctex_result ctex_material_graph_workspace_add_builtin_node(
    ctex_material_graph_workspace* workspace, std::uint32_t owner_kind,
    const char* owner_identifier, const char* type_id, ctex_vec2f position,
    std::uint64_t* out_node_id) {
    return call_material_graph_boundary("ctex_material_graph_workspace_add_builtin_node", [&] {
        if (workspace == nullptr || owner_identifier == nullptr || type_id == nullptr ||
            out_node_id == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "workspace, owner_identifier, type_id and out_node_id are required");
        }
        ctex::graph::GraphWorkspace staged = workspace->value;
        const ctex::graph::NodeId node_id =
            material_graph_workspace_graph(staged, owner_kind, owner_identifier)
                .add_node(ctex::graph::make_builtin_node(type_id, {position.x, position.y}));
        workspace->value = std::move(staged);
        *out_node_id = node_id;
    });
}

extern "C" ctex_result ctex_material_graph_workspace_set_input_value(
    ctex_material_graph_workspace* workspace, std::uint32_t owner_kind,
    const char* owner_identifier, std::uint64_t node_id, const char* input_id,
    const ctex_smart_material_value_descriptor* value) {
    return call_material_graph_boundary("ctex_material_graph_workspace_set_input_value", [&] {
        if (workspace == nullptr || owner_identifier == nullptr || input_id == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "workspace, owner_identifier and input_id are required");
        }
        ctex::graph::GraphWorkspace staged = workspace->value;
        material_graph_workspace_graph(staged, owner_kind, owner_identifier)
            .set_input_value(node_id, input_id, material_graph_value(value));
        workspace->value = std::move(staged);
    });
}

extern "C" ctex_result ctex_material_graph_workspace_set_property_value(
    ctex_material_graph_workspace* workspace, std::uint32_t owner_kind,
    const char* owner_identifier, std::uint64_t node_id, const char* property_id,
    const ctex_smart_material_value_descriptor* value) {
    return call_material_graph_boundary("ctex_material_graph_workspace_set_property_value", [&] {
        if (workspace == nullptr || owner_identifier == nullptr || property_id == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "workspace, owner_identifier and property_id are required");
        }
        ctex::graph::GraphWorkspace staged = workspace->value;
        material_graph_workspace_graph(staged, owner_kind, owner_identifier)
            .set_property_value(node_id, property_id, material_graph_value(value));
        workspace->value = std::move(staged);
    });
}

extern "C" ctex_result ctex_material_graph_workspace_add_link(
    ctex_material_graph_workspace* workspace, std::uint32_t owner_kind,
    const char* owner_identifier, const ctex_material_graph_link_descriptor* link,
    ctex_material_graph_link_info* out_info, char* replaced_source_socket,
    std::size_t replaced_source_socket_size) {
    return call_material_graph_boundary("ctex_material_graph_workspace_add_link", [&] {
        if (workspace == nullptr || owner_identifier == nullptr || link == nullptr ||
            out_info == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "workspace, owner_identifier, link and out_info are required");
        }
        validate_structure_size(link->size, CTEX_MATERIAL_GRAPH_LINK_DESCRIPTOR_V1_SIZE,
                                CTEX_MATERIAL_GRAPH_LINK_DESCRIPTOR_CURRENT_SIZE,
                                "material graph link size");
        ...
        ctex::graph::GraphDocument& graph =
            material_graph_workspace_graph(staged, owner_kind, owner_identifier);
        const ctex::graph::AddLinkResult result = graph.add_link(
            {link->source_node, link->source_socket, link->target_node, link->target_socket});
        const PreparedMaterialGraph prepared = prepare_material_graph(graph);
        return_material_graph_link(prepared, result, *out_info, nullptr, 0, replaced_source_socket,
                                   replaced_source_socket_size);
        workspace->value = std::move(staged);
    });
}

extern "C" ctex_result ctex_material_graph_node_registry_create(
    ctex_material_graph_node_registry** out_registry) {
    return call_material_graph_boundary("ctex_material_graph_node_registry_create", [&] {
        if (out_registry == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "out_registry=null");
        }
        *out_registry = nullptr;
        *out_registry = create_material_graph_node_registry(current_allocator());
    });
}

extern "C" void ctex_material_graph_node_registry_destroy(
    ctex_material_graph_node_registry* registry) {
    if (registry == nullptr) {
        return;
    }
    const ctex_allocator_state allocator = registry->allocator;
    registry->~ctex_material_graph_node_registry();
    deallocate_storage(allocator, registry, sizeof(ctex_material_graph_node_registry),
                       alignof(ctex_material_graph_node_registry));
}

extern "C" ctex_result ctex_material_graph_node_registry_get_info(
    const ctex_material_graph_node_registry* registry,
    ctex_material_graph_node_registry_info* out_info) {
    return call_material_graph_boundary("ctex_material_graph_node_registry_get_info", [&] {
        if (registry == nullptr || out_info == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           registry == nullptr ? "registry=null" : "out_info=null");
        }
        validate_structure_size(out_info->size, CTEX_MATERIAL_GRAPH_NODE_REGISTRY_INFO_V1_SIZE,
                                CTEX_MATERIAL_GRAPH_NODE_REGISTRY_INFO_CURRENT_SIZE,
                                "material graph node registry info size");
        *out_info = {.size = CTEX_MATERIAL_GRAPH_NODE_REGISTRY_INFO_CURRENT_SIZE,
                     .registration_count = registry->value.registrations().size()};
    });
}

extern "C" ctex_result ctex_material_graph_node_registry_register(
    ctex_material_graph_node_registry* registry,
    const ctex_material_graph_host_node_registration_descriptor* descriptor) {
    return call_material_graph_boundary("ctex_material_graph_node_registry_register", [&] {
        if (registry == nullptr || descriptor == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           registry == nullptr ? "registry=null" : "descriptor=null");
        }
        validate_structure_size(descriptor->size,
                                CTEX_MATERIAL_GRAPH_HOST_NODE_REGISTRATION_DESCRIPTOR_V1_SIZE,
                                CTEX_MATERIAL_GRAPH_HOST_NODE_REGISTRATION_DESCRIPTOR_CURRENT_SIZE,
                                "material graph host node registration size");
        ctex::graph::NodeTypeRegistry staged = registry->value;
        staged.register_type(material_graph_host_registration(*descriptor));
        registry->value = std::move(staged);
    });
}

extern "C" ctex_result ctex_material_graph_add_registered_node(
    const ctex_material_graph_node_registry* registry, const void* serialized,
    std::size_t serialized_size, const char* type_id, std::uint32_t type_version,
    ctex_vec2f position, ctex_material_graph_info* out_info, std::uint64_t* out_node_id,
    void* canonical_output, std::size_t canonical_output_size) {
    return call_material_graph_boundary("ctex_material_graph_add_registered_node", [&] {
        if (registry == nullptr || type_id == nullptr || out_info == nullptr ||
            out_node_id == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "registry, type_id, out_info and out_node_id are required");
        }
        validate_structure_size(out_info->size, CTEX_MATERIAL_GRAPH_INFO_V1_SIZE,
                                CTEX_MATERIAL_GRAPH_INFO_CURRENT_SIZE, "material graph info size");
        PreparedMaterialGraph source = prepare_material_graph(serialized, serialized_size);
        const ctex::graph::NodeId node_id = source.graph.add_node(
            registry->value.make_node(type_id, type_version, {position.x, position.y}));
        const PreparedMaterialGraph prepared = prepare_material_graph(std::move(source.graph));
        return_material_graph(prepared, *out_info, canonical_output, canonical_output_size);
        *out_node_id = node_id;
    });
...
extern "C" ctex_result ctex_material_graph_workspace_add_registered_node(
    ctex_material_graph_workspace* workspace, const ctex_material_graph_node_registry* registry,
    std::uint32_t owner_kind, const char* owner_identifier, const char* type_id,
    std::uint32_t type_version, ctex_vec2f position, std::uint64_t* out_node_id) {
    return call_material_graph_boundary("ctex_material_graph_workspace_add_registered_node", [&] {
        if (workspace == nullptr || registry == nullptr || owner_identifier == nullptr ||
            type_id == nullptr || out_node_id == nullptr) {
            throw_boundary(
                CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                "workspace, registry, owner_identifier, type_id and out_node_id are required");
        }
        ctex::graph::GraphWorkspace staged = workspace->value;
        const ctex::graph::NodeId node_id =
            material_graph_workspace_graph(staged, owner_kind, owner_identifier)
...
extern "C" ctex_result ctex_material_graph_validate_registered(
    const ctex_material_graph_node_registry* registry, const void* serialized,
    std::size_t serialized_size, std::uint32_t target,
    const ctex_material_graph_validation_resources_descriptor* resources,
    ctex_material_graph_validation_info* out_info, char* report_output,
    std::size_t report_output_size) {
    return call_material_graph_boundary("ctex_material_graph_validate_registered", [&] {
        if (registry == nullptr || out_info == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           registry == nullptr ? "registry=null" : "out_info=null");
        }
        ...
        const PreparedMaterialGraph prepared = prepare_material_graph(serialized, serialized_size);
        const ctex::graph::GraphValidationReport validation = ctex::graph::validate_graph(
            prepared.graph, registry->value, material_graph_emission_target(target),
            material_graph_validation_resources(resources));
        const std::string report = material_graph_validation_json(validation);
        *out_info = {.size = CTEX_MATERIAL_GRAPH_VALIDATION_INFO_CURRENT_SIZE,
                     .valid = validation.valid() ? 1U : 0U,
                     .error_count = validation.error_count(),
                     .warning_count = validation.warning_count(),
                     .diagnostic_count = validation.diagnostics.size(),
...
extern "C" ctex_result ctex_material_graph_node_registry_verify_contract(
    const ctex_material_graph_node_registry* registry, const char* type_id,
    std::uint32_t type_version, const char* const* pinned_dependencies,
    std::size_t pinned_dependency_count, ctex_material_graph_host_contract_info* out_info,
    char* report_output, std::size_t report_output_size) {
    return call_material_graph_boundary("ctex_material_graph_node_registry_verify_contract", [&] {
        if (registry == nullptr || type_id == nullptr || out_info == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "registry, type_id and out_info are required");
        }
        validate_structure_size(out_info->size, CTEX_MATERIAL_GRAPH_HOST_CONTRACT_INFO_V1_SIZE,
                                CTEX_MATERIAL_GRAPH_HOST_CONTRACT_INFO_CURRENT_SIZE,
                                "material graph host contract info size");
        const std::vector<std::string> pinned = material_graph_resource_names(
            pinned_dependencies, pinned_dependency_count, "pinned_dependencies");
        const ctex::graph::NodeParityReport parity =
            registry->value.verify_parity_fixtures(type_id, type_version);
        const ctex::graph::NodeReplayEligibility replay =
            registry->value.replay_eligibility(type_id, type_version, pinned);
        const std::string report = material_graph_host_contract_json(parity, replay);
        *out_info = {.size = CTEX_MATERIAL_GRAPH_HOST_CONTRACT_INFO_CURRENT_SIZE,
                     .parity_passed = parity.passed() ? 1U : 0U,
                     .parity_failure_count = parity.failures.size(),
                     .replay_eligible = replay.eligible ? 1U : 0U,
                     .unpinned_dependency_count = replay.unpinned_dependencies.size(),
                     .report_size = report.size() + 1};
        validate_string_buffer(report_output, report_output_size, out_info->report_size);
        if (report_output != nullptr) {
            std::memcpy(report_output, report.c_str(), out_info->report_size);
...
extern "C" ctex_result ctex_shader_emit_material(
    ...
    return call_shader_emission_boundary("ctex_shader_emit_material", [&] {
        ...
        return_shader_material_emission(
            prepared, request->target, out_info, vertex_artifact, vertex_artifact_size,
            fragment_artifact, fragment_artifact_size, pass_plan_output, pass_plan_output_size,
            workaround_report_output, workaround_report_output_size);
    });
}

extern "C" ctex_result ctex_shader_emission_cache_create(ctex_shader_emission_cache** out_cache) {
    return call_boundary("ctex_shader_emission_cache_create", [&] {
        if (out_cache == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "out_cache=null");
        }
        *out_cache = create_shader_emission_cache(current_allocator());
    });
}

extern "C" void ctex_shader_emission_cache_destroy(ctex_shader_emission_cache* cache) {
    if (cache == nullptr) return;
    const ctex_allocator_state allocator = cache->allocator;
    cache->~ctex_shader_emission_cache();
    deallocate_storage(allocator, cache, sizeof(ctex_shader_emission_cache),
                       alignof(ctex_shader_emission_cache));
}

extern "C" ctex_result ctex_shader_emission_cache_get_info(
    const ctex_shader_emission_cache* cache, ctex_shader_emission_cache_info* out_info) {
    return call_boundary("ctex_shader_emission_cache_get_info", [&] {
        if (cache == nullptr || out_info == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           cache == nullptr ? "cache=null" : "out_info=null");
        }
        validate_structure_size(out_info->size, CTEX_SHADER_EMISSION_CACHE_INFO_V1_SIZE,
...
extern "C" ctex_result ctex_shader_emission_cache_clear(ctex_shader_emission_cache* cache) {
    return call_boundary("ctex_shader_emission_cache_clear", [&] {
        if (cache == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "cache=null");
        }
...
extern "C" ctex_result ctex_shader_emit_material_cached(
    ctex_shader_emission_cache* cache, const ctex_material_graph_node_registry* registry,
    ...
    std::uint32_t* out_cache_hit) {
    return call_shader_emission_boundary("ctex_shader_emit_material_cached", [&] {
        if (cache == nullptr || request == nullptr || out_info == nullptr ||
            out_cache_hit == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "cache, request, out_info, and out_cache_hit are required");
        }
        validate_structure_size(out_info->size, CTEX_SHADER_MATERIAL_INFO_V1_SIZE,
                                CTEX_SHADER_MATERIAL_INFO_CURRENT_SIZE,
                                "shader material info size");
        ...
        const ctex::emit::MaterialShaderEmissionRequest converted =
            shader_material_request(*request);
        const ctex::emit::CachedMaterialShaderEmission cached =
            registry == nullptr ? cache->material.emit(graph, converted)
                                : cache->material.emit(graph, registry->value, converted);
        const PreparedShaderEmission prepared = prepare_shader_emission(*cached.emission);
        return_shader_material_emission(
            prepared, request->target, out_info, vertex_artifact, vertex_artifact_size,
            fragment_artifact, fragment_artifact_size, pass_plan_output, pass_plan_output_size,
            workaround_report_output, workaround_report_output_size);
...
extern "C" ctex_result ctex_shader_emit_layer_stack(
    ctex_shader_emission_cache* cache, const ctex_shader_layer_stack_request* request,
    ctex_shader_layer_stack_info* out_info, void* artifact_blob, std::size_t artifact_blob_size,
    char* artifact_report_output, std::size_t artifact_report_output_size, char* pass_plan_output,
    std::size_t pass_plan_output_size, char* workaround_report_output,
    std::size_t workaround_report_output_size) {
    return call_shader_emission_boundary("ctex_shader_emit_layer_stack", [&] {
        if (request == nullptr || out_info == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           request == nullptr ? "request=null" : "out_info=null");
        }
        validate_structure_size(out_info->size, CTEX_SHADER_LAYER_STACK_INFO_V1_SIZE,
                                CTEX_SHADER_LAYER_STACK_INFO_CURRENT_SIZE,
                                "shader layer-stack info size");
        const ctex::emit::LayerStackEmissionRequest converted =
            shader_layer_stack_request(*request);
        if (cache == nullptr) {
            const ctex::emit::LayerStackEmission emission = ctex::emit::emit_layer_stack(converted);
            const PreparedLayerStackEmission prepared = prepare_layer_stack_emission(emission);
            return_layer_stack_emission(
                emission, prepared, *request, false, out_info, artifact_blob, artifact_blob_size,
                artifact_report_output, artifact_report_output_size, pass_plan_output,
                pass_plan_output_size, workaround_report_output, workaround_report_output_size);
            return;
        }
        const ctex::emit::CachedLayerStackEmission cached = cache->layer_stack.emit(converted);
        const PreparedLayerStackEmission prepared = prepare_layer_stack_emission(*cached.emission);
        return_layer_stack_emission(*cached.emission, prepared, *request, cached.cache_hit,
                                    out_info, artifact_blob, artifact_blob_size,
...
extern "C" ctex_result ctex_layer_snapshot_create(
    std::uint32_t width, std::uint32_t height,
    const ctex_layer_composite_raster_descriptor* content, std::size_t content_count,
    const ctex_layer_composite_mask_descriptor* masks, std::size_t mask_count,
    ctex_layer_snapshot** out_snapshot) {
    return call_boundary("ctex_layer_snapshot_create", [&] {
        if (out_snapshot == nullptr || (content == nullptr && content_count != 0) ||
            (masks == nullptr && mask_count != 0)) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "declared snapshot inputs and out_snapshot are required");
        }
        *out_snapshot = nullptr;
        ctex::doc::LayerCompositeRequest value =
            make_layer_composite_request(width, height, content, content_count, masks, mask_count);
        *out_snapshot = create_layer_snapshot(current_allocator(), std::move(value));
    });
}

extern "C" void ctex_layer_snapshot_destroy(ctex_layer_snapshot* snapshot) {
    if (snapshot == nullptr) {
        return;
    }
    const ctex_allocator_state allocator = snapshot->allocator;
    snapshot->~ctex_layer_snapshot();
    deallocate_storage(allocator, snapshot, sizeof(ctex_layer_snapshot),
                       alignof(ctex_layer_snapshot));
}

extern "C" ctex_result ctex_layer_snapshot_read(
    const ctex_layer_snapshot* snapshot, ctex_layer_snapshot_info* out_info,
    ctex_layer_snapshot_content_info* content, std::size_t content_capacity,
    ctex_layer_snapshot_mask_info* masks, std::size_t mask_capacity, char* strings,
    std::size_t string_size, ctex_vec4f* pixels, std::size_t pixel_capacity, float* coverage,
    std::size_t coverage_capacity, double* mask_values, std::size_t mask_value_capacity) {
    return call_boundary("ctex_layer_snapshot_read", [&] {
        if (snapshot == nullptr || out_info == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "snapshot and out_info are required");
        }
        validate_structure_size(out_info->size, CTEX_LAYER_SNAPSHOT_INFO_V1_SIZE,
                                CTEX_LAYER_SNAPSHOT_INFO_CURRENT_SIZE, "layer snapshot info size");
        publish_layer_snapshot(snapshot->value, *out_info,
                               {.content = content,
                                .masks = masks,
                                .strings = strings,
                                .pixels = pixels,
                               ...
                               content_capacity, mask_capacity, string_size, pixel_capacity,
                               coverage_capacity, mask_value_capacity);
    });
}

extern "C" ctex_result ctex_texture_set_layer_composite_snapshot_cpu(
    ...
    std::size_t channel_capacity, char* semantic_ids, std::size_t semantic_id_size,
    ctex_vec4f* pixels, std::size_t pixel_capacity) {
    return call_boundary("ctex_texture_set_layer_composite_snapshot_cpu", [&] {
        if (document == nullptr || snapshot == nullptr || out_info == nullptr) {
            throw_boundary(CTEX_RESULT_INVALID_ARGUMENT, CTEX_DIAGNOSTIC_NULL_ARGUMENT,
                           "document, snapshot, and out_info are required");
        }
        validate_structure_size(out_info->size, CTEX_LAYER_COMPOSITE_INFO_V1_SIZE,
                                CTEX_LAYER_COMPOSITE_INFO_CURRENT_SIZE,
                                "layer composite info size");
        ctex::doc::LayerCompositeResult result;
        try {
            result = require_texture_set(*document, texture_set_id).composite_cpu(snapshot->value);
        } catch (const ctex::doc::LayerCompositeError& error) {
            throw_layer_composite_error(error);
        }
        publish_layer_composite(
