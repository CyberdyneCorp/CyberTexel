...
bool cpu_readback_copies_only_named_tiles() {
    ...
    const std::array destinations{
        TileReadbackDestination{version_at(delta.changed_tiles, {0, 0}), first_output},
        TileReadbackDestination{version_at(delta.changed_tiles, {1, 1}), edge_output},
    };
...
bool stale_cpu_readback_does_not_publish() {
    ...
    const std::array destinations{
        TileReadbackDestination{delta.changed_tiles.front(), output},
    };
    TileReadback readback =
        TileReadback::begin_cpu(channels, "pbr.base_color", delta.current_cursor, destinations);
...
bool host_completion_controls_buffer_visibility() {
    ...
    const std::array destinations{
        TileReadbackDestination{host_version({2, 3}, 7), first_output},
        TileReadbackDestination{host_version({4, 5}, 11), second_output},
    };
    ...
    const std::array completed{
        HostTileCompletion{host_version({2, 3}, 7), first_payload},
        HostTileCompletion{host_version({4, 5}, 11), second_payload},
    };
    passed &= expect(
...
bool cancelled_failed_and_mismatched_completions_publish_nothing() {
    ...
    const std::array cancelled_destination{
        TileReadbackDestination{version, cancelled_output},
    };
    TileReadback cancelled = TileReadback::begin_host(cancelled_destination);
    const std::array completion{HostTileCompletion{version, payload}};
    ...
    const std::array failed_destination{TileReadbackDestination{version, failed_output}};
    ...
    const std::array mismatch_destination{TileReadbackDestination{version, mismatch_output}};
    TileReadback mismatch = TileReadback::begin_host(mismatch_destination);
    const std::array wrong_completion{HostTileCompletion{host_version({9, 9}, 3), payload}};
