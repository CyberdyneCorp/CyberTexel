...
Base stroke controls use the documented defaults and inclusive ranges in
[paint-tool parameter validation](paint-tool-parameters.md). Finite values
outside those ranges are clamped before reconstruction;
`StrokeResolver::parameter_report()` records every supplied and resolved value.
