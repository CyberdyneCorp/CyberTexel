...
typedef struct name_hash_entry {
	char   *key;
	name_id value;
} name_hash_entry;

typedef struct kong_core_state {
	type       *types;
	type_id     types_size;
	type_id     next_type_index;
	type_id     function_type_id;
	render_pipelines       all_render_pipelines;
	render_pipeline_groups all_render_pipeline_groups;
	compute_shaders        all_compute_shaders;
	descriptor_set_groups  all_descriptor_set_groups;
	allocated_global allocated_globals[1024];
	size_t           allocated_globals_size;
	uint64_t         next_variable_id;
	bool             kong_error;
	function        *functions;
	function_id      functions_size;
	function_id      functions_zeroed;
	function_id      next_function_index;
	global           globals[1024];
	global_id        globals_size;
	char            *names;
	size_t           names_size;
	name_id          names_index;
	descriptor_set   sets[MAX_SETS];
	size_t           sets_count;
	opcodes          new_code;
	name_hash_entry *hash;
	statement        statements_buffer[4096];
	int              statement_index;
	expression       expressions_buffer[8192];
	int              expression_index;
	token           *tokens_cache;
	size_t           tokens_cache_size;
} kong_core_state;
...
#if defined(_MSC_VER)
#define KONG_THREAD_LOCAL __declspec(thread)
#else
#define KONG_THREAD_LOCAL _Thread_local
#endif

static KONG_THREAD_LOCAL kong_context *active_context = NULL;

#define core_state                  (active_context->core)
#define compiler_types              (core_state->types)
#define compiler_types_size         (core_state->types_size)
#define next_type_index             (core_state->next_type_index)
#define function_type_id            (core_state->function_type_id)
#define all_render_pipelines        (core_state->all_render_pipelines)
#define all_render_pipeline_groups  (core_state->all_render_pipeline_groups)
#define all_compute_shaders         (core_state->all_compute_shaders)
#define all_descriptor_set_groups   (core_state->all_descriptor_set_groups)
#define allocated_globals           (core_state->allocated_globals)
#define allocated_globals_size      (core_state->allocated_globals_size)
#define next_variable_id            (core_state->next_variable_id)
#define kong_error                  (core_state->kong_error)
#define compiler_functions          (core_state->functions)
#define compiler_functions_size     (core_state->functions_size)
#define functions_zeroed            (core_state->functions_zeroed)
#define next_function_index         (core_state->next_function_index)
#define compiler_globals            (core_state->globals)
#define globals_size                (core_state->globals_size)
#define names                       (core_state->names)
#define names_size                  (core_state->names_size)
#define names_index                 (core_state->names_index)
#define compiler_sets               (core_state->sets)
#define compiler_sets_count         (core_state->sets_count)
#define new_code                    (core_state->new_code)
#define hash                        (core_state->hash)
#define statements_buffer           (core_state->statements_buffer)
#define statement_index             (core_state->statement_index)
#define experessions_buffer         (core_state->expressions_buffer)
#define expression_index            (core_state->expression_index)
#define tokens_cache                (core_state->tokens_cache)
#define tokens_cache_size           (core_state->tokens_cache_size)

static void zero_new_function_slots(void) {
	if (compiler_functions_size > functions_zeroed) {
		memset(&compiler_functions[functions_zeroed], 0, (compiler_functions_size - functions_zeroed) * sizeof(function));
		functions_zeroed = compiler_functions_size;
	}
}
...
void error_args(debug_context context, const char *message, va_list args) {
	...
	if (context.filename != NULL) {
		snprintf(prefix, sizeof(prefix), "In column %i at line %i in %s: ", context.column + 1, context.line + 1, context.filename);
	}
	else {
		snprintf(prefix, sizeof(prefix), "In column %i at line %i: ", context.column + 1, context.line + 1);
	}

	size_t prefix_size = strlen(prefix);
	memcpy(active_context->last_error, prefix, prefix_size);
	vsnprintf(&active_context->last_error[prefix_size], sizeof(active_context->last_error) - prefix_size, message, args);
	kong_error = true;
}

void error_args_no_context(const char *message, va_list args) {
	vsnprintf(active_context->last_error, sizeof(active_context->last_error), message, args);
	kong_error = true;
}
...
void functions_init(void) {
	function     *new_functions = realloc(compiler_functions, compiler_functions_size * sizeof(function));
	debug_context context       = {0};
	check(new_functions != NULL, context, "Could not allocate functions");
	compiler_functions = new_functions;
...
static void grow_functions_if_needed(uint64_t size) {
	while (size >= compiler_functions_size) {
		compiler_functions_size *= 2;
		function     *new_functions = realloc(compiler_functions, compiler_functions_size * sizeof(function));
		debug_context context       = {0};
		check(new_functions != NULL, context, "Could not allocate functions");
		compiler_functions = new_functions;
...
function_id add_function(name_id name) {
	...
	compiler_functions[f].name                        = name;
	compiler_functions[f].attributes.attributes_count = 0;
	init_type_ref(&compiler_functions[f].return_type, NO_NAME);
	compiler_functions[f].parameters_size = 0;
	memset(compiler_functions[f].parameter_attributes, 0, sizeof(compiler_functions[f].parameter_attributes));
	compiler_functions[f].block                      = NULL;
	compiler_functions[f].code.size                  = 0;
	compiler_functions[f].descriptor_set_group_index = UINT32_MAX;
	compiler_functions[f].used_builtins              = (builtins){0};
	compiler_functions[f].used_capabilities          = (capabilities){0};
...
function *get_function(function_id function) {
	...
	return &compiler_functions[function];
}

global_id add_global(type_id type, attribute_list attributes, name_id name) {
	uint32_t index            = globals_size;
	compiler_globals[index].name       = name;
	compiler_globals[index].type       = type;
	compiler_globals[index].var_index  = 0;
	compiler_globals[index].value.kind = GLOBAL_VALUE_NONE;
	compiler_globals[index].attributes = attributes;
	compiler_globals[index].sets_count = 0;
	compiler_globals[index].usage      = 0;
	globals_size += 1;
	return index;
}
...
global_id add_global_with_value(type_id type, attribute_list attributes, name_id name, global_value value) {
	...
	compiler_globals[index].type       = type;
	compiler_globals[index].var_index  = 0;
	compiler_globals[index].value      = value;
	compiler_globals[index].attributes = attributes;
	compiler_globals[index].sets_count = 0;
	compiler_globals[index].usage      = 0;
	globals_size += 1;
...
global *find_global(name_id name) {
	for (uint32_t i = 0; i < globals_size; ++i) {
		if (compiler_globals[i].name == name) {
			return &compiler_globals[i];
		}
	}
...
descriptor_set *create_set(name_id name) {
	for (size_t set_index = 0; set_index < compiler_sets_count; ++set_index) {
		if (compiler_sets[set_index].name == name) {
			return &compiler_sets[set_index];
		}
	}

	if (compiler_sets_count >= MAX_SETS) {
		debug_context context = {0};
		error(context, "Max set count of %i reached", MAX_SETS);
		return NULL;
	}

	descriptor_set *new_set = &compiler_sets[compiler_sets_count];
	new_set->name           = name;
	new_set->index          = (uint32_t)compiler_sets_count;
	new_set->globals.size   = 0;

	compiler_sets_count += 1;

	return new_set;
}

descriptor_set *get_set(size_t index) {
	return &compiler_sets[index];
}

size_t get_sets_count(void) {
	return compiler_sets_count;
}
...
void types_init(void) {
	type         *new_types = realloc(compiler_types, compiler_types_size * sizeof(type));
	debug_context context   = {0};
	check(new_types != NULL, context, "Could not allocate types");
	compiler_types  = new_types;
	next_type_index = 0;
...
static void grow_types_if_needed(uint64_t size) {
	while (size >= compiler_types_size) {
		compiler_types_size *= 2;
		type         *new_types = realloc(compiler_types, compiler_types_size * sizeof(type));
		debug_context context   = {0};
		check(new_types != NULL, context, "Could not allocate types");
		compiler_types = new_types;
...
type_id add_type(name_id name) {
	...
	compiler_types[s].name                        = name;
	compiler_types[s].attributes.attributes_count = 0;
	compiler_types[s].members.size                = 0;
	compiler_types[s].built_in                    = false;
	compiler_types[s].array_size                  = 0;
	compiler_types[s].base                        = NO_TYPE;
	compiler_types[s].tex_kind                    = TEXTURE_KIND_NONE;
...
type_id add_full_type(type *t) {
	for (type_id type_index = 0; type_index < next_type_index; ++type_index) {
		if (types_equal(&compiler_types[type_index], t)) {
			return type_index;
		}
	}
...
type_id find_type_by_ref(type_ref *t) {
	...
	for (type_id i = 0; i < next_type_index; ++i) {
		if (compiler_types[i].name == t->unresolved.name) {
			if (compiler_types[i].array_size == t->unresolved.array_size) {
				return i;
			}
			base_type_id = i;
		}
...
kong_builtin_types *kong_active_builtin_types(void) {
	...
	return &active_context->builtins;
}
...
static void initialize_context(kong_context *context) {
	...
	compiler_types_size     = 1024;
	compiler_functions_size = 128;
	names_size              = 1024 * 1024;
	names_index             = 1;
	next_variable_id        = 1;
...
kong_context *kong_context_create(void) {
	...
	context->core = (kong_core_state *)calloc(1, sizeof(kong_core_state));
...
static bool function_owns_unique_code(function_id index) {
	uint8_t *code = compiler_functions[index].code.o;
	if (code == NULL) {
		return false;
	}
	for (function_id earlier = 0; earlier < index; ++earlier) {
		if (compiler_functions[earlier].code.o == code) {
			return false;
		}
	}
	return true;
...
void kong_context_destroy(kong_context *context) {
	...
	kong_context *previous = active_context;
	active_context         = context;
	for (function_id index = 0; index < next_function_index; ++index) {
		if (function_owns_unique_code(index)) {
			free(compiler_functions[index].code.o);
		}
	}
	bool new_code_is_owned = new_code.o != NULL;
	for (function_id index = 0; index < next_function_index; ++index) {
		if (compiler_functions[index].code.o == new_code.o) {
			new_code_is_owned = false;
			break;
		}
	}
	if (new_code_is_owned) {
		free(new_code.o);
	}
	shfree(hash);
	free(tokens_cache);
...
bool kong_context_compile(kong_context *context, const char *source, kong_target target, kong_compilation *compilation) {
	...
	char  *from          = "";
	tokens source_tokens = tokenize(from, source);
	parse(from, &source_tokens);
	resolve_types();
	if (!kong_error) {
		allocate_globals();
		for (function_id index = 0; get_function(index) != NULL; ++index) {
			compile_function_block(&get_function(index)->code, get_function(index)->block);
		}
		analyze();
	}
	if (!kong_error) {
...
const char *kong_context_last_error(const kong_context *context) {
	return context == NULL ? "Kong context is null" : context->last_error;
}
