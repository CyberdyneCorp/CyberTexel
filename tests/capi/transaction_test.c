...
static int bulk_write_round_trip(void) {
    ...
    ctex_channel_region_descriptor region = {
        ...
        .width = 128,
        .height = 64,
        .row_pitch_bytes = 128 * 3,
    };
    ctex_texture_set_transaction* transaction = NULL;
    int ok = expect(
        ctex_texture_set_begin_transaction(value.document, value.set_id, "bulk", targets, 2,
                                           value.snapshot, &transaction) == CTEX_RESULT_SUCCESS,
        "bulk transaction begin failed");
    if (ok) {
        ctex_channel_region_descriptor invalid = region;
        invalid.size = 1;
        ok = expect(ctex_texture_set_transaction_write_region(transaction, "pbr.base_color",
                                                              &invalid, source, sizeof(source)) ==
        ...
        ok = ok && expect(ctex_texture_set_transaction_write_region(
                              transaction, "pbr.base_color", &region, source, sizeof(source) - 1) ==
                              CTEX_RESULT_INVALID_ARGUMENT,
                          "bulk write accepted a short buffer");
        ok = ok && expect(ctex_texture_set_transaction_write_region(
                              transaction, "pbr.base_color", &region, source, sizeof(source)) ==
                              CTEX_RESULT_SUCCESS,
                          "bulk write failed");
        ctex_tile_history_commit_info commit = {.size = CTEX_TILE_HISTORY_COMMIT_INFO_CURRENT_SIZE};
    ...
    size_t required = 0;
    ok = ok &&
         expect(ctex_paint_preview_session_create(value.document, value.set_id, "pbr.base_color",
    ...
    ok = ok && expect(ctex_texture_set_undo_tiles(value.document, value.set_id, &restored) ==
                              CTEX_RESULT_SUCCESS &&
                          restored.copied_pixel_bytes == 0 && restored.exchanged_storage_count == 2,
                      "bulk undo copied pixels");
    transaction = NULL;
    ok = ok && expect(ctex_texture_set_begin_transaction(value.document, value.set_id, "bounded",
                                                         targets, 1, value.snapshot,
                                                         &transaction) == CTEX_RESULT_SUCCESS,
    ...
    ok = ok && expect(ctex_texture_set_transaction_write_region(transaction, "pbr.base_color",
                                                                &region, source, 6) ==
                              CTEX_RESULT_INVALID_ARGUMENT &&
