...
MATRIX = ROOT / "docs" / "paint-tool-scenarios.md"
SPEC = (
    ROOT
    / "openspec"
    / "specs"
...
def main() -> int:
    ...
    if "paint-tool-scenario-matrix" not in labeled:
        failures.append("scenario matrix CTest is missing its capability label")
    if failures:
        for failure in failures:
            print(failure)
        return 1
    print(f"ok: all {len(expected)} paint-tool scenarios have mapped evidence")
    return 0
